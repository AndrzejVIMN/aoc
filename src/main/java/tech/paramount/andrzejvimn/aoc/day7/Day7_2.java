...
public class Day7_2 {
    ...
    public void solve() {
        ...
        System.out.println("Total winnings with Jokers: " + wins);
    }
    ...
    private boolean isFiveOfAKind(String cards) {
        return cards.equals("JJJJJ") || cards.chars().filter(c -> c!= 'J').distinct().count() == 1;
    }

    private boolean isFourOfAKind(String cards) {
        var noJokers = cards.chars().filter(c -> c!= 'J').toArray();
        var jokers = cards.length() - noJokers.length;
        if (jokers == 0) {
        ...
        var distinct = IntStream.of(noJokers).distinct().toArray();
        if (distinct.length != 2) return false;
        var count1 = IntStream.of(noJokers).filter(card -> card == distinct[0]).count();
        var count2 = IntStream.of(noJokers).filter(card -> card == distinct[1]).count();
